...
$(document).ready(function () {
    ...
    function youtubeChannelVedioApiCall(playListId) {
        $.ajax({
            ...
            if (items[0]) {
                ...
                $("#youtube-channel-video .display-watch-video").html('<iframe width="100%" height="619" src="https://www.youtube.com/embed/' + current_videoId + '" frameborder="0" allowfullscreen=""></iframe><h2> ' + items[0].snippet.title + '</h2>');
            }

            grid_video_list = 0;
            videoList = '';
            $.each(items, function (index, e) {
                videoList = videoList + '<div class="col-sm-' + gridPreRowList + ' ycv-list-snippet"><div class="ycv-simple-thumb-wrap"><img class="img-responsive" alt="' + e.snippet.title + '" src="' + e.snippet.thumbnails.medium.url + '"></div><div class="current-video-id" style="display:none">' + e.snippet.resourceId.videoId + '</div><div class="title">' + e.snippet.title + '</div></div>';
            });
    ...
    $('#youtube-channel-pagination #yc-pagination-watch').on('click', '.ycv-list-snippet', function () {
        ...
        $("#youtube-channel-video .display-watch-video").html('<iframe width="100%" height="619" src="https://www.youtube.com/embed/' + currentVideoId + '" frameborder="0" allowfullscreen=""></iframe><h2> ' + video_snippet + '</h2>');
    });
